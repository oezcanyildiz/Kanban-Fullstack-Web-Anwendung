...
export function OrganizationRegisterPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Mock - in real app: POST /api/organization/register mit { organizationName, organizationEmail }
    // Response enthält invitationCode
    const prefix = orgName.replace(/\s+/g, "").toUpperCase().substring(0, 3);
    const mockCode = prefix + "-" + Math.random().toString(36).substring(2, 7).toUpperCase();
          ...
          {!submitted ? (
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <Button type="submit" className="w-full bg-[#0052CC] text-white hover:bg-[#0747A6]">
